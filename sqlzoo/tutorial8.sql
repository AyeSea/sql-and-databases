...
-- 5. Use COALESCE to print the mobile number. Use the number '07986 444 2266' there is no number given. 
...
-- 9. Use CASE to show the name of each teacher 
	 -- followed by 'Sci' if the the teacher is in dept 1 or 2 and 'Art' otherwise.
	 		SELECT name,
	 					 CASE WHEN dept IN (1, 2) THEN 'Sci' ELSE 'Art' END
	 		FROM teacher;

--10. Use CASE to show the name of each teacher 
   -- followed by 'Sci' if the the teacher is in dept 1 or 2 show 'Art' if the dept is 3 and 'None' otherwise.
   		SELECT name,
...
-- More exercises from older version of tutorial 8 (Scottish Parliament)
...
-- 5. Obtain a list of all MSPs by name, give the name of the MSP and the name of the party where available. 
	 -- Be sure that Canavan MSP, Dennis is in the list. Use ORDER BY msp.name to sort your output by MSP.
	 		...
	 		FROM msp LEFT OUTER JOIN party ON (msp.party = party.code)
	 		ORDER BY msp.name;

-- 6. Obtain a list of parties which have MSPs, include the number of MSPs.
		  SELECT party.name, COUNT(*)
		  FROM msp RIGHT OUTER JOIN party on (msp.party = party.code)
